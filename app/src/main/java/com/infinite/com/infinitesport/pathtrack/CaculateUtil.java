...
public class CaculateUtil {
    //绿、粉红、红
    public static int[] colors = new int[]{Color.argb(255, 0, 255, 0), Color.argb(255, 255, 255, 0), Color.argb(255, 255, 0, 0)};

    // 通过设置间隔时间和距离可以控制速度和图标移动的距离
    private static final int TIME_INTERVAL = 80;
    private static final double DISTANCE = 0.000001;
    private static final String TAG = "CaculateUtil";
     ...
     * @return 千米/时
     */
    public static float caculateSpeedWithKm(LatLng start, LatLng end, int interval) {
        Log.i(TAG, "--------distance:" + (double) AMapUtils.calculateLineDistance(start, end));
        double speed = ((double) AMapUtils.calculateLineDistance(start, end)) / 1000 / (((double) interval / 1000) / 3600);
        BigDecimal b = new BigDecimal(speed);
        float f1 = b.setScale(2, BigDecimal.ROUND_HALF_UP).floatValue();
        Log.i(TAG, "caculateSpeed: " + b.toString());
        return f1;
    }
     ...
     * @return 米/秒
     */
    public static float caculateSpeedWithM(LatLng start, LatLng end, int interval) {
        Log.i(TAG, "--------distance:" + (double) AMapUtils.calculateLineDistance(start, end));
        double speed = ((double) AMapUtils.calculateLineDistance(start, end)) / (((double) interval / 1000));
        BigDecimal b = new BigDecimal(speed);
        float f1 = b.setScale(2, BigDecimal.ROUND_HALF_UP).floatValue();
        Log.i(TAG, "caculateSpeed: " + b.toString());
        return f1;
    ...
    /**
     * 根据不同阶段的速度显示轨迹颜色
     * @param interval   经纬度记录点的时间间隔
     * @return
     */
    public static List<Integer> caculateSpeedColor(List<LatLng> list, int interval) {
        List<Integer> colorList = new ArrayList<Integer>();
        for (int i = 0, n = list.size(); i < n; i++) {
            if (i + 1 < n) {
                float s = CaculateUtil.caculateSpeedWithKm(list.get(i), list.get(i + 1), interval);
                Log.i(TAG, "----speed: " + s);
                if (s >= 0 && s < 8) {
     ...
     * @return
     */
    public static PolylineOptions getPolylineOptions(double[] point_data) {
        PolylineOptions  polylineOptions = new PolylineOptions();
        for (int i = 0; i < point_data.length; i += 2) {
            polylineOptions.add(new LatLng(point_data[i + 1], point_data[i]));
        }
        polylineOptions.width(10);      //轨迹宽度
        polylineOptions.color(Color.parseColor("#00000000"));  //轨迹颜色，默认是黑色，当前是透明
     ...
     * @return
     */
    public static List<LatLng> readLatLngs(double[] point_data) {
        List<LatLng> points = new ArrayList<LatLng>();
        for (int i = 0; i < point_data.length; i += 2) {
            points.add(new LatLng(point_data[i + 1], point_data[i]));
        }
        return points;
    }
     ...
     * 根据点获取图标转的角度
     */
    public static double getAngle(Polyline polyline, int startIndex) {
     ...
     * 算取斜率
     */
    public static double getSlope(Polyline polyline, int startIndex) {
        if ((startIndex + 1) >= polyline.getPoints().size()) {
            throw new RuntimeException("index out of bonds");
        }
        LatLng startPoint = polyline.getPoints().get(startIndex);
     ...
     * 测试数据
     */
    public static double[] coords = {116.3499049793749, 39.97617053371078,
